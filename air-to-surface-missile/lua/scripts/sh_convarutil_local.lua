...
function ConvarClass:GiveUIElement(parent)
	if self.type == "bool" then
		...
		hook.Add("TTTUlxModifyAddonSettings", "TTT" .. self.HookLabel .. "ModifySettings", function(name)
			...
			for k, cvar in ipairs(self.ListOfConvars) do
				if not cvar.ttt2 then
					...
					i = i + 1
					tttrslst:AddItem(cvar:GiveUIElement(tttrslst))
				end
			end
			if tttrslst then
			...
			for k, cvar in ipairs(self.ListOfConvars) do
				if cvar.ttt2 then
					...
					i = i + 1
					tttrslst2:AddItem(cvar:GiveUIElement(tttrslst2))
				end
			end
			if tttrslst2 then
