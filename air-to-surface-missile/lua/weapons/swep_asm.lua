...
function SWEP:OnRemove()
	...
	if CLIENT then
		if self.HtmlIcon and self.HtmlIcon:IsValid() then
			self.HtmlIcon:Remove()
		end
		self.HtmlIcon = nil
...
local function CheckFriendly(team, ent)
	debugPrint(
		"Check friendly",
		IsValid(ent) and ent:IsPlayer() and ent:GetName(),
		team,
		ent,
		IsValid(ent),
		ent:IsPlayer(),
...
if SERVER then
	...
	function SWEP:Equip()
		--if not self.ModelC4 then self:SendMessage(0) end
		self.AllowDrop = true
		if IsValid(self:GetOwner()) then
	...
	local function ASMSetVis(ply)
		...
		if IsValid(wep) and wep:GetClass() == "swep_asm" then
			if (wep.Status == 2) or (wep.Status == 3) then
				if IsValid(wep.Camera) then
		...
		if IsValid(inflictor) and inflictor.GetClass then
			...
			if class == "sent_asm" or class == "player" then
				...
				if entID ~= nil then
					...
					if (id == entID) and not GetConVar("ttt_asm_damage_owner"):GetBool() then
						output = output .. " | No damage, because owner damage is disabled!"
						dmginfo:SetDamage(0)
					elseif not GetConVar("ttt_asm_friendlyfire"):GetBool() and CheckFriendly(team, ent) then
...
if CLIENT then
	...
	function SWEP:UpdateStatus(status)
		...
		self.Status = status
		if status == 0 then
			if self.HtmlIcon then
				self.HtmlIcon:SetVisible(true)
			end
			...
			if self.HtmlIcon then
				self.HtmlIcon:SetVisible(false)
			end
			if status == 1 then
		...
		if self.Menu and status > 0 then
			self.Menu:SetVisible(false)
		end
	end
